...
workflow PIPELINE_INITIALISATION {
    ...
    //
    // Create channel from input file provided through params.input for fastas
    //
    Channel
        .fromList(samplesheetToList(params.input, "${projectDir}/assets/schema_input.json"))
        .map {
            meta, fasta -> [ meta.id, file(fasta, checkIfExists: true) ]
        }
        .set { ch_samplesheet }

    //
    // Create poppunk sample sheet
    //
    ch_samplesheet
        .map { it -> tuple( it[0], it[1].fileName)}
        .collectFile{ it ->
            [ "query_list.txt", "${it[0]}\t${it[1]}\n" ]
        }
        .set {ch_query_list}
